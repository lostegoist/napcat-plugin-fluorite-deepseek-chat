...
export function buildConfigSchema(ctx: NapCatPluginContext): PluginConfigSchema {
    return ctx.NapCatConfig.combine(
        ...
        ,
        // DeepSeek 与 AI 配置
        ctx.NapCatConfig.text('apiUrl', 'AI API 地址', 'https://api.deepseek.com', 'DeepSeek API 的基础地址'),
        ctx.NapCatConfig.text('apiKey', 'API Key', '你的API密钥', '用于请求 DeepSeek 的 API Key'),
        ctx.NapCatConfig.text('apiModel', 'API 模型', 'deepseek-chat', '使用的 DeepSeek 模型标识:deepseek-reasoner/deepseek-chat'),
        ctx.NapCatConfig.text('prefix', 'AI 命令前缀', '#ai', '用于 AI 聊天的命令前缀（备用）')
        // TODO: 在这里添加你的配置项
    );
}
